...
import anyaShell from '../assets/anya-shell.png';
...
function Background() {
  ...
  useEffect(() => {
    if (isMounted()) {
      const circleCount = Math.min(width, height) * 0.03;
      setCircles(
  ...
  return (
    <div ref={mouseSroll}>
      <div className="fixed bottom-0 left-0">
        <img src={anya} title="anya" alt="anya" />
        <img
          className="absolute"
          src={leftEye}
          title="anya-left-eye"
          alt="left-eye"
          style={{
            top: `${Math.min(clientY / 20 + 220, 250)}px`,
            left: `${Math.min(clientX / 30 + 200, 230)}px`,
          }}
        />
        <img
          className="absolute"
          src={rightEye}
          title="anya-right-eye"
          alt="right-eye"
          style={{
            top: `${Math.min(clientY / 20 + 200, 228)}px`,
            left: `${Math.min(clientX / 30 + 300, 332)}px`,
          }}
        />
        <img
          src={anyaShell}
      ...
      >
        {lines?.map((value, index) => {
          ...
          const selected = randomChoiceString(['text-primary']);
          return (
            <div
              key={keyName}
              className="absolute"
        ...
        {circles?.map((value, index) => {
          ...
          const selected = randomChoiceString(['text-secondary']);
          return (
            <div key={keyName}>
              <svg
                width={width}
